...
def find_petri_dish(image):
    ...
    median_blur_order = 5
    minmax_radius = (1623/2.0, 2316/2.0)
    centroid = (2570, 1068)
    ...
    centroid_scaled = (int(centroid[0]*scale), int(centroid[1]*scale))
    ...
    smallest_distance_from_centroid = float('inf')


    for i in circles[0,:]:

        a = (i[0] - centroid_scaled[1], i[1]-centroid_scaled[0])
        distance = np.linalg.norm(a)

        if(distance < smallest_distance_from_centroid):
            smallest_distance_from_centroid = distance
            circle = i
...
if __name__=="__main__":
    ...
    circular_mask = find_petri_dish(img)
    ...
    img_masked[idx] = 255;
    cv.imshow("BW2", image_resize(img_masked, height=720));
    cv.waitKey(0)
